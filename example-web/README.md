...
cd example-web
docker compose up -d
docker compose ps   # mysql / redis / rocketmq-namesrv / rocketmq-broker 均 healthy 即可
```
...
```bash
cd example-web
docker compose down -v && docker compose up -d          # 重建全部容器
docker compose ps                                         # 6 个服务全部 healthy

# 关键日志证据（example-web）：Producer 已初始化
...
# → [example-web] 快照已暂存 snapshotId=xxx, sessionId=test-session-001
# → [example-web] RocketMQ 投递 CLAW_SYNTH_TASK 成功 snapshotId=xxx

# 查看 Broker 收到的 Topic（rocketmq 自带 tools 可查）
...
> **注意**：Broker 容器**不挂载 volume 到 `/home/rocketmq/store`**（避免 RocketMQ 5.3.1 的 ScheduleMessageService
> NPE bug），存储用容器内临时目录，适合开发验证；生产环境请用独立部署的 RocketMQ 集群或修改启动脚本显式
> 创建 store 目录结构。
