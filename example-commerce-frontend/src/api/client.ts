...
/** REST 完整对话命令（对应后端 ChatCmd：支持显式指定编排 orchestrationId） */
export interface RunChatCmd {
  sessionId?: string;
  agentId?: string;
...
export const chatApi = {
  /** 完整对话（非流式）：返回完整回复 + 推理轨迹，支持编排选择（routing / marketing / todo-delegate） */
  run(cmd: RunChatCmd): Promise<ChatResponseDTO> {
    return request<ChatResponseDTO>('/agent/chat', {
      method: 'POST',
