package com.mwb.ai.claw.example.commerce.config;
 ...
 * <p>
 * 查找策略复用 {@link ConfigFileLocator}：模块目录（由本类 code source 推导）→
 * 运行目录（user.dir）→ 安装目录 {@code $MWB_AI_CLAW_HOME} → classpath。
...
public class DotenvEnvironmentPostProcessor implements EnvironmentPostProcessor {
    ...
    private Map<String, Object> loadDotenv() {
        ...
        // 查找策略：运行目录（user.dir）→ 安装目录 → classpath（由 ConfigFileLocator 统一）
